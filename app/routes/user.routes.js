      ...
      newUser
        ...
        .then((user) => {
          ...
          const mailOptions = {
            ...
            to: `${user.email}`,
            subject: "Welcome to Geen Core",
            text:
              "\n" +
              "Dear " +
              `${user.name}` +
              ",\n" +
              "\nPlease use the following code to verify your email in Green Core App. Complete the process within one hour of receiving it:\n\n" +
              `${token}\n\n` +
...
router.post(
  ...
  (req, res) => {
    ...
    User.findOne({ email })
      .then((user) => {
        if (user) {
          // if (!user.emailVerify)                       // not allowing  users who hasn't verified email
          //   res
          //     .status(403)
          //     .send({ err: "Please verify your email address before login" });
